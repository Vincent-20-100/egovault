# recall() tier-1 base (Librarian sub-context) Implementation Plan
...
- `core/config.py` — add `generous_limit` to `RecallConfig` (modify, ~line 56-60).
- `config/system.yaml` — surface the new config value (modify, `recall:` block).
- `tools/vault/recall.py` — add `generous: bool = False` param + recall-first branch (modify).
- `tests/tools/vault/test_recall.py` — new tests for generous mode (modify).
- `mcp/server.py` — expose `generous` on the `recall` MCP tool (modify, ~line 144-161).
...
- `docs/user-guide/09-mcp.md` + `docs/user-guide/06-search-and-recall.md` — docs (modify).
...
- Modify: `config/system.yaml` (the `recall:` block)

- [ ] **Step 1: Add the field to RecallConfig**

In `core/config.py`, change the `RecallConfig` class to:
...
In `config/system.yaml`, locate the `recall:` block and add the key (match the existing keys' style/indentation):
...
Run: `python -c "from core.config import RecallConfig; print(RecallConfig().generous_limit)"`
Expected: `20`
...
git commit -m "feat: add recall.generous_limit config for librarian generous mode"
```

---

## Task 2: `generous` mode on recall() (recall-first)

**Files:**
- Modify: `tools/vault/recall.py`
- Test: `tests/tools/vault/test_recall.py`

- [ ] **Step 1: Write the failing tests**

Append to `tests/tools/vault/test_recall.py`:
...
def _ctx_generous(notes, chunks, hybrid_flag=False, generous_limit=20):
    ...
    ctx.settings.system.recall.use_hybrid_retrieval = hybrid_flag
    ctx.settings.system.recall.generous_limit = generous_limit
    return ctx
...
def test_generous_returns_both_tiers_no_escalation_gate():
    ...
    result = recall("q", ctx, generous=True)
...
Run: `python -m pytest tests/tools/vault/test_recall.py -k generous -v`
Expected: FAIL — `recall()` got an unexpected keyword argument `generous`.

- [ ] **Step 3: Implement the generous branch**

Replace the body of `tools/vault/recall.py` `recall()` with:
...
def recall(
    ...
    """Deterministic tiered retrieval. generous=True: recall-first wide net for a sub-agent."""
    cfg = ctx.settings.system.recall
    ...
    note_sources = [
        RecallSource(tier="note", uid=n.note_uid, source_uid=n.source_uid,
                      title=n.title, content=n.content, distance=n.distance)
        for n in sorted(notes, key=lambda r: r.distance)
    ]
    chunk_sources = [
        RecallSource(tier="chunk", uid=c.chunk_uid, source_uid=c.source_uid,
...
- [ ] **Step 5: Run the full recall test file (no regression on tier-0)**

Run: `python -m pytest tests/tools/vault/test_recall.py -v`
Expected: all PASS (existing + 4 new).

- [ ] **Step 6: Commit**

```bash
git add tools/vault/recall.py tests/tools/vault/test_recall.py
git commit -m "feat: add recall-first generous mode to recall() for librarian sub-context"
```

---

## Task 3: Expose `generous` on the MCP recall tool
...
Replace the `recall` MCP tool in `mcp/server.py` with:

```python
@mcp.tool()
def recall(query: str, filters: dict | None = None, limit: int = 5,
           generous: bool = False) -> dict:
    """
    Tiered recall — the preferred entry point for any knowledge question.
    ...
    over search() — it returns tiered signal, not raw top-K noise.
    ...
    result = _recall_tool(query, ctx, filters=search_filters, limit=limit,
                          generous=generous)
...
git commit -m "feat: expose generous mode on the recall MCP tool"
```
...
tools: recall, get_note, get_source, search
---
...
Process:
1. Call `recall(query, generous=true)`. This returns a wide, recall-first pile of
...
The librarian must run in its own context: it calls `recall(query, generous=true)`,
...
def test_librarian_agent_has_frontmatter_and_calls_generous_recall():
...
**Files:**
- Modify: `docs/user-guide/09-mcp.md` and `docs/user-guide/06-search-and-recall.md`
- Modify: `PROJECT-STATUS.md`
- Modify: `SESSION-CONTEXT.md`

- [ ] **Step 1: Document the librarian pattern in the MCP chapter**

In `docs/user-guide/09-mcp.md`, add a section that covers: (a) installing EgoVault as a Claude Code plugin (`/plugin install`) which bundles the MCP server + `librarian` subagent + `/ask-vault` command; (b) the two ways to trigger isolated retrieval — explicit `/ask-vault <question>` (guaranteed) vs. automatic delegation to the `librarian` subagent (heuristic); (c) that this uses the host's own LLM, no API key needed. Match the chapter's existing heading style and tone.

- [ ] **Step 2: Document generous mode in the search/recall chapter**

In `docs/user-guide/06-search-and-recall.md`, add a short subsection: `recall(generous=True)` casts a recall-first wide net (hybrid retrieval, both notes and chunks, untruncated) intended for an isolated agent that does the final triage — as opposed to the default precise mode. Note that precision is then provided by the agent's reasoning.

- [ ] **Step 3: Add the technical-debt entry**
...
- [ ] **Step 4: Note the deferred sampling migration in SESSION-CONTEXT.md**
...
(Baseline per PROJECT-STATUS was 512 passed / 0 skipped.)

- [ ] **Step 2: Run ruff on changed files**

Run: `ruff check tools/vault/recall.py mcp/server.py core/config.py tests/tools/vault/test_recall.py tests/plugin/test_plugin_files.py`
Expected: no errors. Fix any reported.

- [ ] **Step 3: Update PROJECT-STATUS implemented features + session history**

Add a session-history row dated 2026-05-29 summarizing: recall() tier-1 base shipped (generous mode + librarian subagent + /ask-vault command + plugin manifest), no-API-key host-LLM design, sampling deferred. Mark the recall() tier-1 base as done in the relevant list.

- [ ] **Step 4: Commit**

```bash
git add PROJECT-STATUS.md
git commit -m "docs: status update - recall() tier-1 base (librarian) shipped"
```
...
- Keep tier-0 (`generous=False`) behavior byte-for-byte unchanged; the existing recall tests must stay green untouched.
